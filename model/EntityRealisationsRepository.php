...
class EntityRealisationsRepository
{
    private $db;
    public $table;
    public function getDb() // méthode permettant d'instancier la classe PDO et de créer un objet
    {
        if (!$this->db) // seulement si $this->db n'est pas rempli, il n'y a pas de connexion alors on la construit
        {
            ...
            try {
                ...
                $this->table = $xml->table7; // on associe le nom de la table du fichier XML a la propriété de la classe
                try 
...
/************ AJOUTER ET MODIFIER (REMPLACE si id est n'est pas nul) ************/
    ...
    {
        $lien = "../img/" . $_FILES['photo']['name'];
        $requete = "REPLACE INTO realisations (id_realisations,photo,titre,description) VALUES ('$_GET[id]','$lien','$_POST[titre]','$_POST[description]')";
        // var_dump('REPLACE INTO ' . $this->table . '(id_' . $this->table . ',' .  implode(',', array_keys($_POST)). ') VALUES (' . $id . ',' . "'"  . implode("','", $_POST) . "'" . ')');
...
$a = new EntityRealisationsRepository; 
$a->getDb();
