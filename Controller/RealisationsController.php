...
// EXECUTION DES FONCTIONS (qui sont dans EntityRealisationsRepository qu'ont rappel pour les fonctionnalité d'affichage  )
class RealisationsController
{
    private $db;
    public function __construct()
    {
        $this->db = new \model\EntityRealisationsRepository; // permet de récuperer une connexion a la BDD qui se trouve dans le fichier EntityRealisationsRepository.php
    }    
    ...
    // echo '<pre>';print_r($r); echo '</pre>';
        ...
        $this->render('layout.php', 'donnees.php', array(
            'title'=>'Réalisations', // $title  dans l'index
    ...
    public function save($op)
    {
        ...
        $values = ($op == 'update') ? $this->db->select($id) : ''; // si on a envoyé un id dans l'URL, on l'envoi en argument de la méthode select() de EntityRealisationsRepository, cela permettra de selectionner toute les données de l'employé pour les modification.

        //$value = ($op == 'add') ? $this->db->select
        
        //var_dump($values);

        if($_POST)
        {
            $r = $this->db->save(); // lorque l'on valide le formulaire d'ajout, on execute la methode save() du fichier EntityRealisationsRepository .
            $this->redirect('realisations.php');
        }
